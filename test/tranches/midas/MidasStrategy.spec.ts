...
UTest.create({
    ...
    async 'Oracle rate conversion' () {
        ...
        const depositVault = await deployContract<MockDepositVault>(MockDepositVault, [
            mToken.address,
        ]);
        const redemptionVault = await deployContract<MockRedemptionVault>(MockRedemptionVault, [
            mToken.address,
            baseAsset.address,
        ]);
        ...
        const assets = await strategy.convertToAssets(mToken.address, mTokenAmount, 0n);
        ...
        const tokens = await strategy.convertToTokens(mToken.address, baseAmount, 0n);
        ...
        const identity = await strategy.convertToAssets(baseAsset.address, identityAmount, 0n);
    ...
    async 'Rounding: ceil vs floor' () {
        ...
        const tokensFloor = await strategy.convertToTokens(mToken.address, baseAmount, 0n);
        const tokensCeil = await strategy.convertToTokens(mToken.address, baseAmount, 1n);
    ...
    async 'totalAssets oracle staleness check' () {
        ...
        await mToken.$receipt().mint(deployer, strategy.address, 1000n * 10n ** 18n);
    ...
    async 'Oracle: invalid rate reverts' () {
        ...
        // Set oracle to 0
        await oracle.$receipt().setRoundData(deployer, 1n, 0n, BigInt(Math.floor(Date.now() / 1000)));
    ...
    async 'CooldownRequestImpl: isCooldownActive always true' () {
        ...
        const impl = await deployContract<MidasCooldownRequestImpl>(MidasCooldownRequestImpl, [
            baseAsset.address,
            mToken.address,
            redemptionVault.address,
        ]);
