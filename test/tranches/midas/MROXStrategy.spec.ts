...
const test = await $hh.create('mrox');

const { mROX, USDC, oracle, strategy, unstakeCooldown } = await test.deploy();
const { deployer, client } = test;

await test.snapshot('mrox');
...
UTest.create({
    ...
    async 'Oracle rate conversion'() {
        ...
        const assets = await strategy.convertToAssets(mROX.address, mTokenAmount, 0);
        ...
        const tokens = await strategy.convertToTokens(mROX.address, baseAmount, 0);
    ...
    async 'Rounding: ceil vs floor'() {
        ...
        const tokensFloor = await strategy.convertToTokens(mROX.address, baseAmount, 0);
        const tokensCeil = await strategy.convertToTokens(mROX.address, baseAmount, 1);
        ...
        const tinyFloor = await strategy.convertToAssets(mROX.address, 1n, 0);
        const tinyCeil = await strategy.convertToAssets(mROX.address, 1n, 1);
    ...
    async 'totalAssets oracle staleness check'() {
        ...
        await mROX.$receipt().mint(deployer, strategy.address, 1000n * 10n ** 18n);
    ...
    async 'CooldownRequestImpl: isCooldownActive always true'() {
        const impl = new MidasCooldownRequestImpl(await unstakeCooldown.implementations(mROX.address), client);
