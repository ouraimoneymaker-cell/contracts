...
import {MockNAlpha, MockNestAccountant, MockNestVaultPredicateProxy} from "../../contracts/test/nest/MockNestContracts.sol";
import {MockERC20} from "../../contracts/test/MockERC20.sol";

/// @title NestAlphaDeploy
/// @notice Base deployment contract for Nest nALPHA strategy tests
/// @dev Deploys the full Strata stack with mock Nest contracts (no fork required)
contract NestAlphaDeploy is Test {
    // Roles
    ...
    MockNAlpha internal nAlpha;
    MockNestAccountant internal nestAccountant;
    MockNestVaultPredicateProxy internal predicateProxy;
    ...
    NestAlphaStrategy internal strategy;
    Accounting internal accounting;
    NestDepositAdapter internal nestAdapter;
    TrancheDepositor internal depositor;

    // Constants matching mainnet nALPHA
    uint256 constant INITIAL_RATE = 1094524; // ~1.094524 USDC/nALPHA (6 decimals)
    uint256 constant ONE_USDC = 1e6;
    uint256 constant ONE_NALPHA = 1e6;
    ...
    function _deployNestStack() internal {
        vm.startPrank(owner);
        ...
        nAlpha = new MockNAlpha();
        vm.label(address(nAlpha), "nALPHA");

        nestAccountant = new MockNestAccountant(address(usdc), INITIAL_RATE);
        vm.label(address(nestAccountant), "NestAccountant");

        predicateProxy = new MockNestVaultPredicateProxy(nAlpha, nestAccountant);
        ...
        // 2. StrataCDO (nALPHA is the base asset for the CDO)
        StrataCDO cdoImpl = new StrataCDO(IERC20Metadata(address(nAlpha)));
        cdo = StrataCDO(
        ...
        // 5. NestAlphaStrategy
        NestAlphaStrategy strategyImpl = new NestAlphaStrategy(
            IERC20(address(nAlpha)),
            IERC20(address(usdc)),
            INestAccountant(address(nestAccountant))
        );
        strategy = NestAlphaStrategy(
            address(new ERC1967Proxy(
                address(strategyImpl),
                abi.encodeWithSelector(
                    NestAlphaStrategy.initialize.selector,
                    owner,
                    address(acm),
        ...
        vm.label(address(strategy), "NestAlphaStrategy");

        // 6. NestDepositAdapter
        nestAdapter = new NestDepositAdapter(
            INestVaultPredicateProxy(address(predicateProxy)),
            address(0xdead), // nestVault address (mock doesn't use it)
            address(nAlpha)
        );
        vm.label(address(nestAdapter), "NestDepositAdapter");

        // 7. Accounting (6 decimals to match USDC/nALPHA)
        ...
        // Register nALPHA as a supported meta token for both tranches
        // (this is what _deposit_viaAdapter calls after the swap: _deposit(vault, nALPHA, ...))
        ...
        TrancheDepositor.TAdapterConfig memory adapterConfig = TrancheDepositor.TAdapterConfig({
            adapter: IAdapter(address(nestAdapter)),
            tokenOut: address(nAlpha),
            minimumReturnPercentage: 900 // 90% min output (rate > 1.0 means fewer nALPHA per USDC)
        });
    ...
    function _mintNAlpha(address to, uint256 amount) internal {
        nAlpha.mint(to, amount);
    }
    ...
    function _depositToJrt(address user, uint256 amount) internal {
        vm.startPrank(user);
        IERC20(address(nAlpha)).approve(address(jrtVault), amount);
        jrtVault.deposit(address(nAlpha), amount, user);
        vm.stopPrank();
    }

    function _depositToSrt(address user, uint256 amount) internal {
        vm.startPrank(user);
        IERC20(address(nAlpha)).approve(address(srtVault), amount);
        srtVault.deposit(address(nAlpha), amount, user);
        vm.stopPrank();
    }
}
