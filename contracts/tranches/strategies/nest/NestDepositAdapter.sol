...
/// @title NestDepositAdapter
/// @notice Adapter that converts USDC to nALPHA via the NestVaultPredicateProxy
...
contract NestDepositAdapter is IAdapter {
    ...
    /// @notice The NestVault address for USDC deposits on this chain
    address public immutable nestVault;

    /// @notice The nALPHA share token address
    address public immutable nALPHA;

    error UnexpectedTokenIn(address token);
    ...
    function swap(
        ...
        if (tokenOut != nALPHA) revert UnexpectedTokenOut(tokenOut);
        ...
        // Approve the NestVaultPredicateProxy to spend the deposit asset
        ...
        uint256 sharesBefore = IERC20(nALPHA).balanceOf(address(this));

        // Call NestVaultPredicateProxy.deposit
        predicateProxy.deposit(
            tokenIn,
            amountIn,
            address(this),   // recipient = adapter, then forward to caller
            nestVault,
            predicateMessage
        );

        amountOut = IERC20(nALPHA).balanceOf(address(this)) - sharesBefore;
        if (amountOut < minAmountOut) revert InsufficientOutput(amountOut, minAmountOut);

        // Transfer nALPHA shares to the recipient (TrancheDepositor)
        SafeERC20.safeTransfer(IERC20(nALPHA), recipient, amountOut);
    }
}
