...
UTest.create({
    ...
    async 'totalAssets oracle staleness check' () {
        ...
        await mHYPER.$receipt().mint(deployer, strategy.address, 1000n * 10n ** 18n);
        ...
        const freshResult = await strategy.totalAssets(
            latestNav,
        ...
        const staleResult = await strategy.totalAssets(
            latestNav,
            oracleUpdatedAt + 100n,
        );
    ...
    async 'CooldownRequestImpl: isCooldownActive always true' () {
        ...
        const impl = new MidasCooldownRequestImpl(
            await unstakeCooldown.implementations(mHYPER.address),
            client
        );
