...
const { factory, deployer, client } = $hh.test;

async function deployContract<T extends ContractBase>(
  Contract: any,
  args: any[],
): Promise<T> {
...
UTest.create({
  ...
  async "Oracle rate conversion"() {
    // Deploy mocks
    const mToken = await deployContract("MockMToken", []);
    const baseAsset = await deployContract("MockBaseAsset", []);
    ...
    // convertToAssets: 100 mToken at $1.05 = 105 baseAsset
    ...
    l`100 mToken => cyan<${$bigint.toEther(assets, 18)}> baseAsset`;
    $require.eq(
      assets,
      105n * 10n ** 18n,
    ...
    // convertToTokens: 105 baseAsset at $1.05 = 100 mToken
    const baseAmount = 105n * 10n ** 18n;
    const tokens = await strategy.convertToTokens(
      mToken.address,
      baseAmount,
      0n,
    );
    l`105 baseAsset => cyan<${$bigint.toEther(tokens, 18)}> mToken`;
    $require.eq(
      tokens,
      100n * 10n ** 18n,
      "105 baseAsset at $1.05 = 100 mToken",
    );
